use arbitrage_monitor::{
    arbitrage::OrderBookArbitrageDetector, binance::BinanceMonitor, raydium::RaydiumAmmMonitor,
    task::TasksRunner,
};
use std::sync::Arc;
...
async fn await_ctrl_c(tracker: TasksRunner, token: CancellationToken) {
    ...
    spawn(async move {
        match signal::ctrl_c().await {
            ...
            Err(error) => {
                error!(?error, "failed to install Ctrl-C handler; cancelling tasks");
            }
        }
